...
5. **`UNSET_LONG` precision regression net.** `packages/ibkr/src/const.ts:12` defines `UNSET_LONG = BigInt(2 ** 63) - 1n`. v4 originally suspected this was lossy; Phase 1b verified empirically that the constant is precise (rounding direction in `Number(2 ** 63)` and `-1n` coincidentally cancel — see §6.1 erratum). Phase 1b ships a test pinning this. Rust ports of i64 fields must still reconstruct `i64::MAX` from a canonical string, since the cancellation does not generalize to other sentinels (e.g., a future u64 variant). See §6.1 caveats.
...
**`UNSET_LONG` JS precision caveat (erratum, see Phase 1b test).** [packages/ibkr/src/const.ts:12](../packages/ibkr/src/const.ts:12) defines `UNSET_LONG = BigInt(2 ** 63) - 1n`. v4 originally asserted this was lossy because `2 ** 63` exceeds `Number.MAX_SAFE_INTEGER`. **Phase 1b verified empirically that the constant is precise**: `Number(2 ** 63) === 9223372036854775808` exactly (an exact power-of-two double), so `BigInt(2 ** 63) - 1n === 9223372036854775807n === canonical i64::MAX`. The Number rounding direction and the `-1n` step coincidentally cancel. **The constant is precise, but only by coincidence** — the cancellation does NOT generalize to other sentinels (e.g., `BigInt(2 ** 64) - 1n` for a u64 sentinel does NOT yield `u64::MAX`). Phase 1b ships `src/domain/trading/__test__/unset-long-precision.spec.ts` as a regression net pinning the current precise behavior; any future change to the constant must re-verify. Rust ports of i64 fields must still reconstruct `i64::MAX` from the canonical string, not from arithmetic on JS Numbers.
...
| 22 | (Not addressed.) Suspected `UNSET_LONG = BigInt(2 ** 63) - 1n` JS precision bug. | v4 §6.1 caveats + Phase 1b Deliverable 5 regression test. **Erratum:** Phase 1b verified the constant is precise (rounding direction cancels with `-1n`). Test pins current behavior; canonical-string reconstruction still required for non-power-of-two i64 sentinels. | `packages/ibkr/src/const.ts:12`; `src/domain/trading/__test__/unset-long-precision.spec.ts` |
