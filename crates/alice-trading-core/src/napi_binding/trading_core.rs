...
impl TradingCore {
    ...
    pub async fn init_uta(&self, account_config: AccountConfig) -> napi::Result<()> {
        ...
        let mut state = UtaState::restore_or_new(
        ...
        // registered via subscribe_events(), calls it NonBlocking. If not yet
        // registered the event is silently dropped (ring buffer retains it
        // for backfill).
    ...
    /// receives a Shutdown command and terminates cleanly.
    #[napi]
