...
import type { AnyUta } from '../uta-manager.js'
...
export async function buildSnapshot(
  ...
  try {
    // TS-backed UTA: use broker-direct path with full order data
    if (isTsUta(uta)) {
      return await _buildTsSnapshot(uta, trigger)
    }

    // Rust-backed proxy: derive from exported state (no live broker query)
    const [accountInfo, positions, gitStatus] = await Promise.all([
      uta.getAccount(),
      uta.getPositions(),
      uta.status(),
    ])
    ...
    return {
      ...
      positions: positions.map(p => ({
        aliceId: p.contract.aliceId ?? p.contract.symbol ?? 'unknown',
        currency: p.currency,
        side: p.side,
...
async function _buildTsSnapshot(
